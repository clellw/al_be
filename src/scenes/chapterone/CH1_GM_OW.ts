...
import { CH1_CU_OW_2 } from "./CH1_CU_OW_2";


export class CH1_GM_OW {
    scene: Scene;
    engine: Engine;
    Player!: AbstractMesh;
    ...
    async CreateMap(): Promise<void> {
        ...
        tvscript.push("On the TV, there's a broadcast about the latest innovations of our company in the Tech industry.")
        tvscript.push("I always wondered what was the point of promoting our own products inside our own office.")
        tvscript.push("The add is of high quality even.")
        tvscript.push("/")

        const counterscript: string[] = [];

        counterscript.push("There's no one at the counter today. I think the hostess has her day off.")
        counterscript.push("...In fact, we're in the middle of spring. most people have their day, or rather their week off.")
        counterscript.push("/")

        const podonescript: string[] = [];

        podonescript.push("This is what I consider as my desktop.")
        podonescript.push("I call it a pod, but raphaël is actually the one who gave it its name.")
        podonescript.push("It's called the 'Rather Wonderfull Query Function System For Alternative Solution, Xain Corporate'.")
        podonescript.push("Or RWQFSFASXC for *short*.")
        podonescript.push("I tried all I could to change the name.")
        podonescript.push("And I ended up remembering the full name, to my own demise.")
        podonescript.push("/")

        const podtwoscript: string[] = [];

        podtwoscript.push("This one isn't mine. It's still unused to this day.")
        podtwoscript.push("It's covered in dust, even though it's only been here for a year.")
        podtwoscript.push("/")

        const boxscript: string[] = [];

        boxscript.push("The cleaning service doesn't bother to clean this room anymore.")
        boxscript.push("The boxes keep on accumulating, even though there's never anything new in this room.")
        boxscript.push("/")

        const screenscript: string[] = [];

        screenscript.push("At how many screens can you consider yourself happy in your life?")
        screenscript.push("/")
        ...
        setTimeout(() => {
            this.scene.registerBeforeRender(() => {
                ...
                else if (triggerElUp.intersectsMesh(this.Player)) {
                    key.alpha = Math.min(1, key.alpha + 0.05);
                    if (eventKeyStatus.e && this.canMove) {
                        elev_arrived.play()
                        this.canMove = false;
    ...
    async naratorWriter(block: GUI.Rectangle, portraitBox: GUI.Rectangle, portrait: GUI.Image, narator: GUI.TextBlock, script: string, index: number, na: StaticSound): Promise<void> {
        ...
        if (index <= script.length) {
            if (script.startsWith("Or") || script.startsWith("...") || script.startsWith("At") || script.startsWith("...") || script.startsWith("I always") || script.startsWith("The boxes") || script.startsWith("And I ended")) { portrait.sourceLeft = 74; portrait.sourceTop = 0 }
            if (script.startsWith("I tried")) { portrait.sourceLeft = 74; portrait.sourceTop = 74 }
            if (script.startsWith("The add")) { portrait.sourceLeft = 0; portrait.sourceTop = 0 }
    ...
    nextScene() {
        const next = new CH1_CU_OW_2(this.canvas);
        this.engine.stopRenderLoop();
        if (this.scene) {
            this.scene.dispose();
        }
