...
import { CH2_CU_UN_1 } from "./CH2_CU_UN_1";


export class CH2_CU_OW_2 {
    scene: Scene;
    engine: Engine;
    Player!: AbstractMesh;
    ...
    async createPlayer(): Promise<void> {
        ...
        this.Player.position = new Vector3(3.5, 10, -19);
    ...
    async CreateDialog(): Promise<void> {
        ...
        const sideCamera = new FreeCamera("SideCamera", new Vector3(1, 10.7, -16), this.scene);
        ...
        script.push("Hello Helia.")
        script.push("Hello Raphaël.")
        script.push("Not a very warm reunion, if I dare say.")
        script.push("Saying hello is already enough.")
        script.push("I'm really confident today.")
        script.push("I promise you, it will be a lot more interesting this time!")
        script.push("uh huh...")
        script.push("Are you listening to me?")
        script.push("I am. You said: 'it will be a lot more interesting this time'.")
        script.push("I will take your words for what they are and be mad at you if there's nothing new.")
        script.push("Anyway, I'm ready.")
        script.push("Still as direct as ever.")
        script.push("Okay, we can start when you want.")
        script.push("Just one more thing.")
        script.push("You may encounter a familiar face this time around.")
        script.push("Is this forshadowing something?")
        script.push("Take my words as they are and be mad at me if there's nothing new.")
        script.push("Okay. Then I better not be double mad at you.")
        script.push("!Raphaël is still optimistic about all of this.")
        script.push("!I was even more than him when we started working together.")
        script.push("!He's never shown a single frown or disatisfaction when working on this project.")
        script.push("!I genuinely admire him for staying optimistic after all this time.")
        script.push("!If only I could have been as tenacious.")
        script.push("Time to end this.")
        script.push("/")
        ...
        this.scene.onBeforeRenderObservable.add(() => {
            if (keyStatus["e"]) {
                ...
                if (this.canSkip) {
                    ...
                    if (script[script_index].startsWith("/")) {
                    ...
                    else if (script_index == 2 || script_index == 4) {
                        dialogBox.isVisible = false;
                        portraitBox.isVisible = false;
                        typing.play();
                        setTimeout(() => {
                        ...
                        typing.play();
                        sideCamera.rotation = new Vector3(0, 3*Math.PI/4, 0);
                        sideCamera.position = new Vector3(1, 10.7, -17);
                        setTimeout(() => {
                            portrait.alpha = 0.5;
                            se_message.play();
                            speaker_index++;
                        ...
                        if (script_index == 5 || script_index == 17) {
                        ...
                        if (script_index == 13) {
                            sideCamera.rotation = new Vector3(0, -5*Math.PI/9, 0);
                            sideCamera.position = new Vector3(5, 10.7, -18);
                        }
    ...
    nextScene() {
        const next = new CH2_CU_UN_1(this.canvas);
        this.engine.stopRenderLoop();
        if (this.scene) {
            this.scene.dispose();
        }
