...
import { CH3_CU_OW_2 } from "./CH3_CU_OW_2";


export class CH3_GM_OW {
    scene: Scene;
    engine: Engine;
    Player!: AbstractMesh;
    ...
    async CreateMap(): Promise<void> {
        ...
        const triggerVessel = MeshBuilder.CreateSphere("triggerVessel", { diameter: 1.5 });
        triggerVessel.visibility = 0;
        triggerVessel.position = new Vector3(0.1, 10.5, -21.5);
        ...
        tvscript.push("Okay so now I am CERTAIN that this TV is never turned off.")
        tvscript.push("Been a whole month of the same add on the same screen in the same office.")
        tvscript.push("Can't figure out what's even the purpose of this, but there must be one.")
        tvscript.push("...")
        tvscript.push("I think.")
        tvscript.push("...")
        tvscript.push("I hope...")
        tvscript.push("/")

        const counterscript: string[] = [];

        counterscript.push("...")
        counterscript.push("Welcome to Xain Corporate's business office! Are you here for-")
        counterscript.push("for...")
        counterscript.push("Urgh! How does she do it?")
        counterscript.push("I could never greet anyone like that...")
        counterscript.push("I kind of envy her now.")
        counterscript.push("/")

        const podonescript: string[] = [];

        podonescript.push("...")
        podonescript.push("What's your name again?")
        podonescript.push("Did I finally forgot it?")
        podonescript.push("...")
        podonescript.push("...")
        podonescript.push("I remember it...")
        podonescript.push("/")

        const podtwoscript: string[] = [];

        podtwoscript.push("At first, I thought this one was for Raphaël to dive into the simulation.")
        podtwoscript.push("He would not have been the best influence for our models, sure...")
        podtwoscript.push("But it could have been fun to dive with someone else.")
        podtwoscript.push("/")

        const boxscript: string[] = [];

        boxscript.push("I think the boxes stopped accumulating at some point.")
        boxscript.push("There is the exact same number as the last time I counted th...")
        boxscript.push("Why did I count them in the first place?")
        boxscript.push("/")

        const screenscript: string[] = [];

        screenscript.push("Is it just what a normal desktop looks like at this point?")
        screenscript.push("Was I sleeping in my fridge for the past decade and I woke up in a world where there are more screens than persons?")
        screenscript.push("/")
        
        const vesselscript: string[] = [];

        vesselscript.push("Wh-!?")
        vesselscript.push("Is it what I think it is?")
        vesselscript.push("It looks just like Flusselle...")
        vesselscript.push("Recognizing her out of the simulation makes me a little uncomfortable.")
        vesselscript.push("/")
        ...
        setTimeout(() => {
            this.scene.registerBeforeRender(() => {
                ...
                else if (triggerVessel.intersectsMesh(this.Player)) {
                    key.alpha = Math.min(1, key.alpha + 0.05);
                    if (eventKeyStatus.e) {
                        if (this.canSkip) {
                            portraitBox.isVisible = true;
                            ...
                            this.canSkip = false;
                            this.canMove = false;
                            this.naratorWriter(dialogBox, portraitBox, portrait, message, vesselscript[this.paragraph], 0, na_message);
                        }
                    }
                }
                else {
        ...
        const obs14 = MeshBuilder.CreateBox("obs14");
        obs14.position = new Vector3(0.1, 10, -21.5);
        obs14.scaling = new Vector3(0.3, 1, 0.3);
        obs14.checkCollisions = true;
        obs14.isVisible = false;
    }
    ...
    async naratorWriter(block: GUI.Rectangle, portraitBox: GUI.Rectangle, portrait: GUI.Image, narator: GUI.TextBlock, script: string, index: number, na: StaticSound): Promise<void> {
        ...
        if (index <= script.length) {
            if (script.startsWith("Did anyone") || script.startsWith("Why did I") || script.startsWith("Is it just") || script.startsWith("Can't figure")) { portrait.sourceLeft = 74; portrait.sourceTop = 0 }
            if (script.startsWith("Urgh!") || script.startsWith("I hope") || script.startsWith("I remember")) { portrait.sourceLeft = 74; portrait.sourceTop = 74 }
            if (script.startsWith("for...")) { portrait.sourceLeft = 74; portrait.sourceTop = 37 }
            if (script.startsWith("Wh-!?") || script.startsWith("What's your")) { portrait.sourceLeft = 0; portrait.sourceTop = 37 }
            if (script.startsWith("It looks") || script.startsWith("I kind of")) { portrait.sourceLeft = 0; portrait.sourceTop = 0 }
    ...
    nextScene() {
        const next = new CH3_CU_OW_2(this.canvas);
        this.engine.stopRenderLoop();
        if (this.scene) {
            this.scene.dispose();
        }
